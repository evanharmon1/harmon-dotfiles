...
that keep the sidebar meaningful. Install it with `brew install herdr` or the installer at
https://herdr.dev/ (the server auto-starts on the first `herdr` invocation); the
