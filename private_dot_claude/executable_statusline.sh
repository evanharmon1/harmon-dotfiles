#!/usr/bin/env bash
# statusline.sh — Claude Code `statusLine` renderer for the local machine.
#
...
# Managed by chezmoi at ~/.claude/statusline.sh and wired up by
# ~/.claude/settings.json, so the renderer and its configuration travel
