#!/usr/bin/env bash
# statusline.sh — Antigravity CLI `statusLine` renderer for the local machine.
#
...
#
# Antigravity CLI pipes the session JSON on stdin and renders whatever we print.
# Managed by chezmoi at ~/.gemini/antigravity-cli/statusline.sh and wired up by
# ~/.gemini/antigravity-cli/settings.json, so the renderer and its configuration
# travel together across machines.
#
...
# stripped inside jq: Antigravity CLI renders our stdout as ANSI, and a directory,
